...
public class MusicaRepositoryImpl implements MusicaRepository {
  ...
  public Optional<Musica> acharPeloId(final UUID id) {
    return jpaRepository.findById(id).map(entity -> mapearComArtistas(List.of(entity)).getFirst());
  }
  ...
  public void atualizar(
      final UUID id, final AtualizarMusicaRequest request, final String caminhoDoArquivo) {
    ...
    if (request.tagIds() != null || request.generoMusicalIds() != null) {
      final var tagsIds =
          request.tagIds() == null
              ? entity.getTags().stream().map(item -> item.getTag().getId()).toList()
              : request.tagIds();
      final var generoMusicaisIds =
          request.generoMusicalIds() == null
              ? entity.getGenerosMusicais().stream()
  ...
  private void atualizarRelacionamentos(
      final MusicaEntity entity, final List<UUID> tagIds, final List<UUID> generoMusicalIds) {
    ...
    tags.forEach(
        tag -> entity.getTags().add(MusicaDaTagEntity.builder().musica(entity).tag(tag).build()));

    entity.getGenerosMusicais().clear();
    generosMusicais.forEach(
        generoMusical ->
            entity
                .getGenerosMusicais()
                .add(
                    GeneroDaMusicaEntity.builder()
                        .musica(entity)
  ...
  private List<GeneroMusicalEntity> buscarGeneros(final List<UUID> generoMusicalIds) {
    ...
    final List<GeneroMusicalEntity> generosMusicais =
        generoMusicalJpaRepository.findAllById(generoMusicalIds);
  ...
  private List<Musica> mapearComArtistas(final List<MusicaEntity> entities) {
    final List<Musica> musicas = entities.stream().map(mapper::toDomain).toList();
    final List<UUID> artistaIds =
        musicas.stream()
    ...
    final Map<UUID, UsuarioEntity> artistasPorId =
        usuarioJpaRepository.findAllById(artistaIds).stream()
