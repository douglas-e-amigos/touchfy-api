...
public class StreamingMusicaUseCase {
  ...
  public StreamingMusicaResponse execute(final UUID id, final String rangeHeader) {
    ...
    final ArquivoRecuperadoResponse arquivo =
        buscarArquivoUseCase.execute(musica.getCaminhoDoArquivo());
    final long tamanhoTotal = arquivo.conteudo().length;

    if (tamanhoTotal == 0) {
      return new StreamingMusicaResponse(
          arquivo.nome(), arquivo.contentType(), arquivo.conteudo(), 0, 0, 0, false);
    }
    ...
    return new StreamingMusicaResponse(
        arquivo.nome(),
        arquivo.contentType(),
        Arrays.copyOfRange(arquivo.conteudo(), (int) intervalo.inicio(), (int) intervalo.fim() + 1),
        intervalo.inicio(),
        intervalo.fim(),
        tamanhoTotal,
        intervalo.parcial());
  ...
  private Range parseRange(final String rangeHeader, final long tamanhoTotal) {
    ...
    if (!rangeHeader.startsWith("bytes=")) {
      throw new ResponseStatusException(
          HttpStatus.REQUESTED_RANGE_NOT_SATISFIABLE, "Range inválido.");
    }
    ...
    try {
      ...
      if (limites[0].isBlank()) {
        ...
        fim =
            limites[1].isBlank()
                ? tamanhoTotal - 1
                : Math.min(Long.parseLong(limites[1]), tamanhoTotal - 1);
      }
      ...
      throw new ResponseStatusException(
          HttpStatus.REQUESTED_RANGE_NOT_SATISFIABLE, "Range inválido.", exception);
    }
  }
