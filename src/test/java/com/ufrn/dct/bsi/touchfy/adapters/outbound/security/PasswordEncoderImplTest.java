...
class PasswordEncoderImplTest {
  ...
  @Mock private org.springframework.security.crypto.password.PasswordEncoder springPasswordEncoder;
  ...
  void deveRetornarTrueQuandoSenhasCoincidem() {
    ...
    when(springPasswordEncoder.matches(senhaPura, senhaCriptografada)).thenReturn(true);
  ...
  void deveRetornarFalseQuandoSenhasNaoCoincidem() {
    ...
    when(springPasswordEncoder.matches(senhaPura, senhaCriptografada)).thenReturn(false);
  ...
  void deveDelegarChamadaParaPasswordEncoderDoSpring() {
    ...
    verify(springPasswordEncoder, times(1)).matches(senhaPura, senhaCriptografada);
  }
}
