...
class DeletarPlaylistUseCaseTest {

    @Test
    void deveDeletarPlaylistQuandoUsuarioForDono() {
        final PlaylistRepository repository = mock(PlaylistRepository.class);
        final DeletarPlaylistUseCase useCase = new DeletarPlaylistUseCase(repository);
        ...
        useCase.execute(playlistId, donoId);

        verify(repository).deletar(playlistId);
    }
