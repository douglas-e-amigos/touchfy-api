...
class RefreshTokenRepositoryImplTest {
    ...
    void deveAcharTokenPeloValor() {
        ...
        final RefreshTokenEntity entity = RefreshTokenEntity.builder()
                .token(token)
                .revogado(false)
                .expiracao(Instant.now())
                .build();

        when(jpaRepository.findByToken(token))
                .thenReturn(Optional.of(entity));

        final Optional<RefreshTokenEntity> resultado =
                repository.acharPeloToken(token);
    ...
    void deveSalvarRefreshToken() {
        ...
        when(usuarioMapper.toEntity(usuario))
