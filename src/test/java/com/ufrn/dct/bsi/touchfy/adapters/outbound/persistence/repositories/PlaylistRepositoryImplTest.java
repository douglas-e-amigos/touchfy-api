...
class PlaylistRepositoryImplTest {
  ...
  void deveCriarPlaylistComConvidados() {
    ...
    final PlaylistMusicaJpaRepository playlistMusicaJpaRepository =
        mock(PlaylistMusicaJpaRepository.class);
    ...
    final var request =
        new CriarPlaylistRequest(
            "Minha Playlist", "Desc", Visibilidade.PUBLICA, List.of(convidadoId));

    when(usuarioJpaRepository.findAllById(List.of(convidadoId)))
        .thenReturn(List.of(convidadoEntity));
  ...
  void deveAdicionarMusicaNaPlaylist() {
    ...
    final var playlistEntity =
        PlaylistEntity.builder()
            ...
            .build();
    final var musicaEntity =
        MusicaEntity.builder().id(musicaId).nome("Música").caminhoDoArquivo("path").build();
  ...
  void deveRetornarPlaylistsVisiveisParaUsuario() {
    ...
    final var protegida =
        PlaylistEntity.builder()
            .id(UUID.randomUUID())
            .nome("Protegida")
            .visibilidade(Visibilidade.PROTEGIDA)
            .donoId(UUID.randomUUID())
            .usuariosConvidados(Set.of(UsuarioEntity.builder().id(usuarioId).build()))
            .build();

    when(jpaRepository.findAll())
        .thenReturn(List.of(publica, privadaDoUsuario, privadaDeOutro, protegida));
