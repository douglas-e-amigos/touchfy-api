...
class RefreshTokenUseCaseTest {
    ...
    void deveGerarNovoAccessToken() {
        ...
        final RefreshTokenEntity refreshEntity = criarRefreshTokenEntity(
                false,
        ...
        when(tokenService.isRefreshTokenValid(refreshToken))
                .thenReturn(true);

        when(tokenService.getRefreshSubject(refreshToken))
                .thenReturn(username);

        when(usuarioRepository.acharPeloNomeDeUsuario(username))
                .thenReturn(Optional.of(usuarioEntity));

        when(usuarioMapper.toDomain(usuarioEntity))
                .thenReturn(usuarioDomain);

        when(tokenService.generateAccessToken(usuarioDomain))
                .thenReturn(novoAccessToken);
        ...
        verify(tokenService)
                .getRefreshSubject(refreshToken);

        verify(usuarioRepository)
                .acharPeloNomeDeUsuario(username);

        verify(usuarioMapper)
                .toDomain(usuarioEntity);

        verify(tokenService)
                .generateAccessToken(usuarioDomain);
    }
    ...
    void deveLancarExcecaoQuandoRefreshTokenEstiverRevogado() {
        ...
        final RefreshTokenEntity entity = criarRefreshTokenEntity(
                true,
                Instant.now().plusSeconds(3600)
        );
