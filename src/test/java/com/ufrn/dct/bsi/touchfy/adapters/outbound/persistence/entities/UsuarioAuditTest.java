...
@DataJpaTest(properties = {"spring.jpa.hibernate.ddl-auto=none", "spring.flyway.enabled=true"})
@Import({
...
public class UsuarioAuditTest {
  ...
  void DevePreencherCamposDeCriacaoUsuarioAudit() {
    ...
    assertThat(salvo.getCriadoPor()).isEqualTo(UUID.nameUUIDFromBytes("sistema".getBytes()));
  ...
  void DevePreencherCamposDeAlteracaoUsuarioAudit() throws InterruptedException {
    ...
    assertThat(usuarioAtualizado.getAtualizadoPor())
        .isEqualTo(UUID.nameUUIDFromBytes("sistema".getBytes()));
  ...
  void DeveFazerSoftDeleteComAuditoriaCompleta() {
    ...
    assertThat(deletado.getDeletadoPor()).isEqualTo(UUID.nameUUIDFromBytes("admin".getBytes()));
  }
