...
class ValidarTamanhoArquivoTest {
  ...
  void setUp() {
    ...
    final TamanhoArquivoValido annotation = mock(TamanhoArquivoValido.class);

    when(annotation.tamanhoArquivo()).thenReturn(TamanhoArquivo.FOTO_DE_PERFIL);
  ...
  void deveRetornarVerdadeiroParaArquivoComTamanhoValido() {
    ...
    final MultipartFile arquivo = new MockMultipartFile("foto", "foto.png", "image/png", conteudo);
  ...
  void deveRetornarFalsoParaArquivoMaiorQueTamanhoMaximo() {
    final byte[] conteudo = new byte[11 * 1024 * 1024];
  ...
  void deveAceitarArquivoExatamenteNoTamanhoMinimo() {
    final byte[] conteudo = new byte[500 * 1024];
  ...
  void deveAceitarArquivoExatamenteNoTamanhoMaximo() {
    final byte[] conteudo = new byte[10 * 1024 * 1024];
  ...
  void deveLancarExcecaoQuandoTamanhoMinimoForMaiorQueTamanhoMaximo() {
    final ValidarTamanhoArquivo validator = new ValidarTamanhoArquivo();

    final TamanhoArquivo tamanhoArquivo = mock(TamanhoArquivo.class);

    when(tamanhoArquivo.getTamanhoMinimo()).thenReturn(1000L);

    when(tamanhoArquivo.getTamanhoMaximo()).thenReturn(100L);

    final TamanhoArquivoValido annotation = mock(TamanhoArquivoValido.class);

    when(annotation.tamanhoArquivo()).thenReturn(tamanhoArquivo);

    validator.initialize(annotation);

    final MultipartFile arquivo =
        new MockMultipartFile("foto", "foto.png", "image/png", new byte[500]);

    final RuntimeException exception =
        assertThrows(
            RuntimeException.class,
            () -> validator.isValid(arquivo, mock(ConstraintValidatorContext.class)));
